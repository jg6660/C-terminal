...
void ExecuteExtract(char *token,const char delemiter[])
{
	char temp2[1000]="unzip ";
	char temp3[1000]="tar xvzf ";
	char temp4[1000]="tar xvf ";
	if(strcmp(token, "extractzip") == 0)
	{
	token = strtok(NULL,delemiter);
	strcat(temp2,token);
	system(temp2);
	}
	else if(strcmp(token, "extractgz") == 0)
	{
	token = strtok(NULL,delemiter);
	strcat(temp3,token);
	system(temp3);
	}
	else if(strcmp(token, "extracttar") == 0)
	{
	token = strtok(NULL,delemiter);
	strcat(temp4,token);
	system(temp4);
